...
const SuccessPage = async ({
  ...
  return (
    // 2. Add dir={locale === "ar" ? "rtl" : "ltr"} for proper Arabic alignment
    <div
        ...
        <Card className="border-border/40 bg-card/50 shadow-xl">
          ...
          <CardContent className="space-y-6">
            ...
            <section className="bg-my-main/5 border border-my-main/20 rounded-lg p-4">
              <h3 className="font-bold mb-3 text-center">{t("stepsTitle")}</h3>
              <div className="grid grid-cols-3 gap-3 text-center">
                {[
                  { icon: Mail, label: t("steps.email") },
                  { icon: Package, label: t("steps.ship") },
                  { icon: Home, label: t("steps.shop") },
                ].map((step, i) => (
          ...
          <CardFooter className="pt-4">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3 w-full">
              <Button asChild variant="outline" className="border-border">
                {/* 3. Ensure links include the locale if not using a custom Link wrapper */}
                <Link
                  href={`/${locale}/`}
